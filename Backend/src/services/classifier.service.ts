...
export function classifyMessage(message: string): ClassificationResult {
  ...
  for (const row of allKeywords) {
    const kw = (row.keyword || '').toString().toLowerCase();

    // Exact substring match (existing behaviour)
    if (kw && (normalized.includes(kw) || kw.includes(normalized))) {
      scores[row.category_name] = (scores[row.category_name] || 0) + row.weight;
      continue;
    }

    // Token-aware soft match: check if any token in the user message
    // appears in the keyword or vice-versa (handles 'sleep' vs 'sleeping')
